...
export function initOptimizer(): void {
  ...
  function createPieceRow(id = '', width = '', height = '', qty = 1) {
    ...
    (tr.querySelector('.rm') as HTMLButtonElement).addEventListener('click', () => {
      tr.remove();
      triggerAutoCalc();
  ...
  function createStockRow(id = '', width = '', height = '', qty = 0, price = 0, unlimited = true) {
    ...
    unlimitedChk.addEventListener('change', () => {
      qtyInput.disabled = unlimitedChk.checked;
    });
  ...
  function updateJsonFromTables() {
    ...
    const obj = {
      pieces: data.pieces,
      stocks: data.stocks,
      kerf_mm: parseFloat((document.getElementById('kerf') as HTMLInputElement).value) || 0,
      objective: (document.getElementById('objective') as HTMLSelectElement).value,
    };
  ...
  function populateTablesFromInput(obj: any) {
    ...
    for (const p of pieces)
      createPieceRow(p.id || p.piece_id || '', p.width_mm || p.width || 0, p.height_mm || p.height || 0, p.quantity || 1);
  ...
  function parsePiecesText(text: string) {
    ...
    for (const raw of lines) {
      ...
      let qty = 1;
      let dimToken = '';
      if (parts.length >= 2 && /^\d+$/.test(parts[0]) && /[x×X]/.test(parts[1])) {
        qty = parseInt(parts[0]);
        dimToken = parts[1];
      } else if (parts.length >= 1 && /[x×X]/.test(parts[0])) {
        dimToken = parts[0];
      } else continue;
    ...
    if (added) {
      updateJsonFromTables();
      triggerAutoCalc();
      setStatus(`Imported ${added} piece lines`);
    }
    return added;
  }
  ...
  function parseStocksText(text: string) {
    ...
    if (added) {
      updateJsonFromTables();
      triggerAutoCalc();
      setStatus(`Imported ${added} stock lines`);
    }
    return added;
  }
  ...
  function saveConfig() {
    try {
      ...
      localStorage.setItem('woodcut_config', JSON.stringify(obj));
  ...
  function loadSaved() {
    try {
      ...
      if (!raw) {
        setStatus('No saved config');
        return;
      }
      const obj = JSON.parse(raw);
      populateTablesFromInput(obj);
      setStatus('Saved config loaded');
      triggerAutoCalc();
    } catch {
      setStatus('Failed to load saved config');
    }
  }
  ...
  function triggerAutoCalc() {
    if (chkAutoCalc.checked) {
      ...
      autoCalcTimer = setTimeout(() => btnCalc.click(), 350);
    } else {
      updateJsonFromTables();
    }
  }
    ...
    if (!worker) {
      ...
      worker.addEventListener('message', (e: MessageEvent) => {
        ...
        if (msg.type === 'result') {
          ...
          const stockSummary = Object.entries(sizeCounts)
            .map(([size, count]) => `  ${size} mm quantity: ${count}`)
            .join('\n');
          ...
          else if (p && typeof p === 'object' && p.percent != null) pct = p.percent;
          setProgress(pct);
        }
      });
    }
  ...
  btnCalc.addEventListener('click', () => {
    const input = (() => {
      try {
        return JSON.parse(inputArea.value);
      } catch {
        return null;
      }
    })();
    ...
    if (chkUseWorker.checked) {
      ...
      setStatus('Solving (worker)...');
    } else {
      try {
        const result = solve(input, (prog: any) => {
          let pct = 0;
          if (typeof prog === 'number') pct = prog;
          else if (prog && typeof prog === 'object' && prog.percent != null) pct = prog.percent;
          setProgress(pct);
        });
        lastSolution = result.solution;
        renderSolution(svgContainer, lastSolution, {});
    ...
    try {
      localStorage.setItem('woodcut_config', inputArea.value);
    } catch {
      // ignore
    }
  });
  ...
  document.getElementById('btn-export-json')!.addEventListener('click', () => {
    if (!lastSolution) return alert('No solution to export');
    const b = new Blob([JSON.stringify(lastSolution, null, 2)], { type: 'application/json' });
    downloadBlob(b, 'solution.json');
  });
  document.getElementById('btn-export-csv')!.addEventListener('click', () => {
    if (!lastSolution) return alert('No solution to export');
    const rows = ['board_index,stock_id,piece_id,width_mm,height_mm,x_mm,y_mm'];
    for (const b of lastSolution.boards) {
      for (const p of b.placements || []) {
        rows.push([b.board_index, b.stock_id, p.piece_id, p.width_mm, p.height_mm, p.x_mm, p.y_mm].join(','));
      }
    }
  ...
  document.getElementById('btn-export-svg')!.addEventListener('click', () => {
    if (!lastSolution) return alert('No solution to export');
    const svg = getSvgString(svgContainer, lastSolution, {});
  ...
  if (typeof window !== 'undefined' && window.addEventListener) {
    window.addEventListener('beforeprint', () => {
      if (lastSolution)
        renderSolution(svgContainer, lastSolution, { gapMultiplier: 1.3, strokeMultiplier: 1.6, patternScale: 3.0, labelFontSize: 20 });
    });
